...
4. 保持以下约束不变，除非用户明确要求修改：
   ...
   - 当前模板中至少要把首页五个核心入口做成独立详情页：`今日運勢`、`心靈樹洞`、`塔羅測算`、`星盤合盤`、`靈魂伴侶`
  ...
  - `十二星座` Banner 不应继续走通用详情页，应拆成专门的星座人格介绍页，集中展示 12 星座的人格特质介绍；陪伴页中其下方还应补充 `十二生肖人格地圖` 入口，并进入独立生肖人格介绍页；在 `十二生肖` 下方还应补充 `塔羅牌解讀` 入口，并进入独立的塔羅牌百科详情页，覆盖大阿爾克那与小阿爾克那说明
...
- 至少校验首页的 `今日運勢`、`心靈樹洞`、`塔羅測算`、`星盤合盤`、`靈魂伴侶` 已接到独立详情 Activity
- 至少校验陪伴页的 `熱門房間`、`占卜測算`、`療癒夜話` 已接到独立详情 Activity
- 至少校验首页与陪伴页的 `十二星座` Banner 已接到独立的星座人格详情 Activity，且陪伴页已增加独立的 `十二生肖人格地圖` 入口与详情 Activity，并在其下方增加独立的 `塔羅牌解讀` 入口与详情 Activity
